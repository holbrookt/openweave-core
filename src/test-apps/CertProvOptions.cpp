...
enum
{
    ...
    kWeaveDeviceKeyBufSize            = 128,    // Size of buffer needed to hold Weave device private key.
...
static uint64_t sOperationalDeviceId = kNodeIdNotSpecified;

static EncodedECPrivateKey sOperationalDevicePrivKey;
static uint8_t sOperationalDevicePrivateKey[kWeaveDeviceKeyBufSize];
static uint16_t sOperationalDevicePrivateKeyLength = 0;

static uint8_t sOperationalDeviceCert[kWeaveDeviceCertBufSize];
static uint16_t sOperationalDeviceCertLength = 0;

static uint8_t sOperationalDeviceRelatedCerts[kWeaveDeviceCertBufSize];
static uint16_t sOperationalDeviceRelatedCertsLength = 0;
...
void CertProvClientEventHandler(void * appState, WeaveCertProvEngine::EventType eventType, const WeaveCertProvEngine::InEventParam & inParam, WeaveCertProvEngine::OutEventParam & outParam)
{
    ...
    case WeaveCertProvEngine::kEvent_ResponseReceived:
    {
        if (inParam.ResponseReceived.ReplaceCert)
        {
            ...
            VerifyOrExit(certLen <= sizeof(sOperationalDeviceCert), err = WEAVE_ERROR_BUFFER_TOO_SMALL);
            memcpy(sOperationalDeviceCert, cert, certLen);
            sOperationalDeviceCertLength = certLen;

            // Store device intermediate certificates related to the device certificate.
            VerifyOrExit(relatedCertsLen <= sizeof(sOperationalDeviceRelatedCerts), err = WEAVE_ERROR_BUFFER_TOO_SMALL);
            memcpy(sOperationalDeviceRelatedCerts, relatedCerts, relatedCertsLen);
            sOperationalDeviceRelatedCertsLength = relatedCertsLen;
        }
        else
        {
            if (binding != NULL)
